...
import { buildTotpAuthUrl, generateTotpSecretBase32 } from "@/server/auth/totp";
import { encryptSecret } from "@/server/auth/secret-box";
...
export async function POST(request: Request) {
  ...
  const limited = await rateLimit({ key: `mfa:totp:setup:${access.userId}:${ip}`, limit: 10, windowMs: 60_000 });
  if (!limited.ok) {
