...
export async function POST(request: Request) {
  ...
  const limited = await rateLimit({ key: `mfa:totp:enable:${access.userId}:${ip}`, limit: 10, windowMs: 60_000 });
  if (!limited.ok) {
  ...
  if (!user?.mfaTotpSecret) return NextResponse.json({ error: "not_setup" }, { status: 409 });
  ...
  await prisma.user.update({
    where: { id: access.userId },
    data: { mfaTotpEnabledAt: new Date() }
  });
