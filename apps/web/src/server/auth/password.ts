...
function scrypt(password: string, salt: Buffer, keylen: number, params: ScryptParams): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    crypto.scrypt(
      ...
      {
        ...params,
        // The stronger work factor exceeds Node's default scrypt memory cap.
...
export function passwordHashNeedsRehash(stored: string) {
  const parsed = parseStoredHash(stored);
  if (!parsed) return false;

  return (
    parsed.params.N < DEFAULT_PARAMS.N ||
    parsed.params.r < DEFAULT_PARAMS.r ||
    parsed.params.p < DEFAULT_PARAMS.p ||
